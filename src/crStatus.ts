...
const formatStatusLabel = (primary: string, secondary?: string): string => {
...
const isStandardKubeCondition = (condition: GenericCondition): boolean =>
  typeof condition.type === 'string' && typeof condition.status === 'string';

const isPhaseCondition = (condition: GenericCondition): boolean =>
...
const pickLatestCondition = (conditions: GenericCondition[]): GenericCondition | null => {
  if (conditions.length === 0) {
    return null;
  }

  return [...conditions].sort(
    (left, right) => conditionTimestamp(right) - conditionTimestamp(left),
  )[0];
};

const getTopLevelStatus = (
