...
import { K8sModelRef, resolveKindToModelKey, resolveRefModelKey, ResourceRef } from './pageContext';
import {
...
export const isPlausibleResourceName = (
  ...
  if (isVerticalListNoiseLine(name)) {
    return false;
  }
...
export const extractRefsFromEventsDocument = (parsed: unknown): ResourceRef[] => {
  ...
  for (const event of events) {
    ...
    const namespace =
      involved.Namespace ?? involved.namespace ?? record.Namespace ?? record.metadata?.namespace;
...
export const normalizeResourceRef = (
  ...
  return {
    ...
    ...(ref.useProjectRoute ? { useProjectRoute: true } : {}),
  };
};
...
export const extractResourcesFromText = (
  ...
  const podsNamespace = tools
    ? Object.values(tools).find((tool) => tool.name === 'pods_list_in_namespace')?.args?.namespace
    : undefined;
  const podNamespace = typeof podsNamespace === 'string' ? podsNamespace : defaultNamespace;
...
export const extractResourceRefs = (
  ...
  if (tools) {
    Object.values(tools).forEach((tool) => {
      ...
      extractResourcesFromToolContent(tool.content, tool.name, args).forEach((ref) => {
        addRef(ref);
      });
