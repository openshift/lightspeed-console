...
const resourceRows = '[data-test-rows="resource-row"]';
...
const filterByName = async (page: Page, name: string) => {
  const input = page.locator('[data-test="name-filter-input"]');
...
const goToPodDetails = async (page: Page, ns: string, podName: string) => {
  ...
  const row = page.locator(resourceRows).filter({ hasText: podName });
...
const responseAction = `${userFeedbackSel} .ols-plugin__response-action`;
...
test.describe('OLS UI', () => {
  ...
  test.describe.serial('Streamed response', { tag: ['@response'] }, () => {
    test('Test submitting a prompt and fetching the streamed response', async ({ page }) => {
      ...
      await expect(page.locator(popover)).toContainText('Waiting for LLM provider...');
    ...
    test('Test response with error, partial response text and tool call', async ({ page }) => {
      ...
      await expect(aiEntry.locator('.pf-v5-c-label').filter({ hasText: 'ABC' })).toBeVisible();
    });
  });
  ...
  test.describe.serial('Copy to clipboard', { tag: ['@clipboard'] }, () => {
    ...
    test('Test copy conversation functionality', async ({ page, context }) => {
      ...
      await expect(page.locator(tooltip)).toContainText('Copy conversation');

      await page.locator(copyConversationButton).click();

      await expect(page.locator(tooltip)).toContainText('Copied');
      await expect(page.locator(tooltip)).toContainText('Copy conversation', {
        timeout: 3000,
      });
  ...
  test.describe.serial('Attach menu', { tag: ['@attach'] }, () => {
    test('Test attach options on pods list page', async ({ page }) => {
      ...
      await expect(page.locator('[data-test-rows="resource-row"]').first()).toBeVisible({
        timeout: 2 * MINUTE,
      });
      const rowCount = await page.locator('[data-test-rows="resource-row"]').count();
    ...
    test('Test modifying attached YAML', async ({ page }) => {
      ...
      await expect(m.locator('.monaco-editor').first()).toBeVisible();
      await expect(m.locator('.monaco-editor').first()).toContainText(podNamePrefix);
