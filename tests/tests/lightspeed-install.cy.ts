      ...
      if (Cypress.env('CONSOLE_IMAGE')) {
        cy.exec(
          ...
          if (result.stderr === '') {
            ...
            cy.exec(
              `oc patch ${csvname} --namespace=${OLS.namespace} --type='json' -p='[{"op": "replace", "path": "/spec/install/spec/deployments/0/spec/template/spec/containers/0/args/9", "value":"--console-image-pf5=${Cypress.env('CONSOLE_IMAGE')}"}]' --kubeconfig ${Cypress.env('KUBECONFIG_PATH')}`,
            );
            cy.exec(
