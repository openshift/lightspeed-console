...
v1 Namespace openshift-lightspeed Active 169m pod-security.kubernetes.io/enforce=restricted`;
  ...
  it('extracts projects from projects_list with project route', () => {
    const tableContent = `APIVERSION KIND NAME STATUS AGE
v1 Project payments Active 3h
v1 Project shared-services Active 1d`;
    ...
    strictEqual(refs.length, 2);
    strictEqual(refs[0].kind, 'Namespace');
    strictEqual(refs[0].name, 'payments');
    strictEqual(refs[0].useProjectRoute, true);
    strictEqual(buildResourceConsolePath(refs[0], testK8sModels), '/k8s/cluster/projects/payments');
  });
...
describe('isPlausibleResourceName', () => {
  it('rejects age values such as 5m', () => {
    strictEqual(isPlausibleResourceName('5m'), false);
    strictEqual(isPlausibleResourceName('72m'), false);
  });
});
