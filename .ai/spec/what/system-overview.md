...
16. [PLANNED: OLS-2700] The plugin will support an interaction mode selector
    ...
    The existing request-body `mode` field (currently `ask` |
    `troubleshooting` — see chat.md rule 6) remains unchanged until
    OLS-2700 ships; when it does, `agent` will be added as a third valid
    value. Non-PF6 builds continue to expose only the attachment-menu mode
    toggle (rules 14–15) and never send `mode: agent`.
